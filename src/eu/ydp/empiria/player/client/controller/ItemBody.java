...
public class ItemBody implements IActivity, IStateful, InternalEventsListener, WidgetWorkflowListener {
	...
	public ItemBody(DisplayContentOptions options, ModuleSocket moduleSocket, final InteractionEventsListener interactionEventsListener){
		...
		moduleEventsListener = new IModuleEventsListener() {
			...
			public void onTouchStart(com.google.gwt.dom.client.Element target,
					int pageX, int pageY) {
				onInternalEvent(new InternalEvent(target, Event.ONMOUSEDOWN, pageX, pageY));
			...
			public void onTouchMove(com.google.gwt.dom.client.Element target,
					int pageX, int pageY) {
				onInternalEvent(new InternalEvent(target, Event.ONMOUSEMOVE, pageX, pageY));
			...
			public void onStateChanged(boolean procesFeedback, IInteractionModule sender){
				interactionEventsListener.onStateChanged(new StateChangedInteractionEvent(procesFeedback, sender));
			}

			@Override
			public void onSoundPlay(String url) {
				interactionEventsListener.onFeedback(new FeedbackSoundInteractionEvent(url));
			}
