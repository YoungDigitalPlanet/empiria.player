 ...
 * <p/>
 * For each method in the factory interface, the binder will determine the implementation type from the return type and the provided bindings. It will then
 * check all constructors in the implementation type against the method parameters (taking named {@literal @}{@link Assisted} annotations into account) and pick
 * a matching one (if available). To inject the selected constructor the binder will write a common method injection, replacing the parameters commonly
 * retrieved through key-specific getter methods with values obtained from the matching method arguments. Finally, after an instance has been constructed, it
 * will be member-injected before it is returned to the caller of the method.
 */
public class FactoryBinding extends AbstractBinding implements Binding {
    ...
    /**
     * Collection of all implementations produced by this factory, each annotated with @Assisted. This is used to gather all required member-inject methods.
     */
    ...
    public SourceSnippet getCreationStatements(NameGenerator nameGenerator, List<InjectorMethod> methodsOutput) throws NoSourceNameException {
        ...
        for (AssistData assisted : assistData) {
            ...
            sb.append("\n\n    ").append(signature).append(" {").append("\n      return ").append(assistedCreateCall).append(";").append("\n    }"); // End
            // method.
        }
    ...
    private SourceSnippet callAssistedCreate(AssistData assisted, NameGenerator nameGenerator, List<InjectorMethod> methodsOutput) throws NoSourceNameException {
        ...
        SourceSnippet assistedInjectMethodBody = new SourceSnippetBuilder().append(returnTypeName).append(" result = ").append(methodCall).append("\n")
                .append(memberInjectCall).append("\n" + postConstruct(assisted.implementation)).append("\nreturn result;").build();
    ...
    private String postConstruct(TypeLiteral<?> implementation) {
        StringBuilder builder = new StringBuilder();
        Method[] declaredMethods = implementation.getRawType().getDeclaredMethods();
        for (int i = 0; i < declaredMethods.length; i++) {
            Method method = declaredMethods[i];
            if (method.isAnnotationPresent(PostConstruct.class)) {
                builder.append("//PostContruct method\n");
                builder.append("result.").append(method.getName()).append("();");
                builder.append("\n");
            }
        }
    ...
    /**
     * Matching logic for {@literal @}{@link AssistedInject} constructor and method parameters.
     * <p/>
     * This returns true if and only if all @Assisted parameters in the constructor exactly match (in any order) all @Assisted parameters the method's
     * parameter.
     */
    ...
    private static class AssistData {
        ...
        private AssistData(TypeLiteral<?> implementation, MethodLiteral<?, Constructor<?>> constructor, MethodLiteral<?, Method> method, String[] parameterNames) {
