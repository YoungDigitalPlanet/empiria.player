...
/**
 * Writes the definition of a single fragment of the Ginjector. A Ginjector fragment contains all the code to create objects bound in that Ginjector that belong
 * to a particular package, exposing only methods to create objects that are not package-private.
 * <p/>
 * <p/>
 * Visible for testing, so it can be returned from a mock factory.
 */
class GinjectorFragmentOutputter {
    ...
    private String postConstruct(Key<?> key) {
        StringBuilder builder = new StringBuilder();
        Method[] declaredMethods = key.getTypeLiteral().getRawType().getDeclaredMethods();
    ...
    void commit() {
        ...
        sourceWriteUtil.writeMethod(writer, String.format("public %s(%s injector)", fragmentClassName, ginjectorClassName), "this.injector = injector;");

        if (hasEagerSingletonInitialization()) {
            // Write a method to initialize eager singletons.
            sourceWriteUtil.writeMethod(writer, "public void initializeEagerSingletons()", initializeEagerSingletonsBody.toString());
        }
    ...
    interface Factory {
        GinjectorFragmentOutputter create(GinjectorBindings bindings, FragmentPackageName fragmentPackageName,
                                          @Assisted("ginjectorPackageName") String ginjectorPackageName, @Assisted("ginjectorClassName") String ginjectorClassName);
    }
}
