...
/**
 * A mutually-exclusive selection radio button widget. Fires {@link com.google.gwt.event.dom.client.ClickEvent}s when the radio button is clicked, and
 * {@link ValueChangeEvent}s when the button becomes checked. Note, however, that browser limitations prevent ValueChangeEvents from being sent when the radio
 ...
 * </p>
 */
public class AccessibleRadioButton extends AccessibleCheckBox {
    ...
    /**
     * Creates a new radio associated with a particular group, and initialized with the given HTML label. All radio buttons associated with the same group name
     ...
     */
    public AccessibleRadioButton(String name, String label) {
        this(name);
        setText(label);
    }

    /**
     * Creates a new radio button associated with a particular group, and initialized with the given label (optionally treated as HTML). All radio buttons
     ...
     */
    public AccessibleRadioButton(String name, String label, boolean asHTML) {
        this(name);
        if (asHTML) {
            setHTML(label);
        } else {
     ...
     * Radio buttons are grouped by their name attribute, so changing their name using the setName() method will also change their associated group.
     * <p/>
     * If changing this group name results in a new radio group with multiple radio buttons selected, this radio button will remain selected and the other radio
     * buttons will be unselected.
     *
     * @param name name the group with which to associate the radio button
     */
    @Override
    ...
    /**
     * No-op. CheckBox's click handler is no good for radio button, so don't use it. Our event handling is all done in {@link #onBrowserEvent}
     */
    @Override
