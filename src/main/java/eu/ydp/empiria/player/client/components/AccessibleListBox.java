 ...
 * This widget is capable of automatically adjusting its direction according to its content. This feature is controlled by {@link #setDirectionEstimator}, and
 * is off by default.
 * </p>
 * 
 * <h3>Use in UiBinder Templates</h3>
 * <p>
 * The items of a ListBox element are laid out in &lt;g:item> elements. Each item contains text that will be added to the list of available items that will be
 * shown, either in the drop down or list. (Note that the tags of the item elements are not capitalized. This is meant to signal that the item is not a runtime
 * object, and so cannot have a <code>ui:field</code> attribute.) It is also possible to explicitly specify item's value using value attribute as shown below.
 * <p>
 * For example:
 * 
 * <pre>
 ...
 * <b>Subclasses should neither read nor write option text directly from the option elements created by this class, since such text may need to be wrapped in
 * Unicode bidi formatting characters. They can use the getOptionText and/or setOptionText methods for this purpose instead.</b>
 */
@SuppressWarnings("deprecation")
public class AccessibleListBox extends FocusWidget implements SourcesChangeEvents, HasChangeHandlers, HasName, HasDirectionEstimator {
	 ...
	 * @return list box
	 */
	public static AccessibleListBox wrap(Element element) {
		// Assert that the element is attached.
		assert Document.get().getBody().isOrHasChild(element);

		AccessibleListBox listBox = new AccessibleListBox(element);
	 ...
	 */
	public AccessibleListBox(boolean isMultipleSelect) {
	 ...
	 */
	protected AccessibleListBox(Element element) {
		super(element);
	...
	/**
	 * Gets the currently-selected item. If multiple items are selected, this method will return the first selected item ({@link #isItemSelected(int)} can be
	 * used to query individual items).
	 * 
	 ...
	 *            the text of the item to be inserted
	 * @param dir
	 *            the item's direction. If {@code null}, the item is displayed in the widget's overall direction, or, if a direction estimator has been set, in
	...
	/**
	 * {@inheritDoc} See note at {@link #setDirectionEstimator(com.google.gwt.i18n.shared.DirectionEstimator)}.
	 */
	@Override
	 ...
	 * 
	 * <p>
	 * Note that setting the selection programmatically does <em>not</em> cause the {@link ChangeHandler#onChange(ChangeEvent)} event to be fired.
	 * </p>
	 * 
	 * @param index
	 ...
	 * 
	 * After calling this method, only the specified item in the list will remain selected. For a ListBox with multiple selection enabled, see
	 * {@link #setItemSelected(int, boolean)} to select multiple items at a time.
	 * 
	 * <p>
	 * Note that setting the selected index programmatically does <em>not</em> cause the {@link ChangeHandler#onChange(ChangeEvent)} event to be fired.
	 * </p>
	 * 
	 * @param index
	...
	/**
	 * Sets the value associated with the item at a given index. This value can be used for any purpose, but is also what is passed to the server when the list
	 * box is submitted as part of a {@link FormPanel}.
	 * 
	 * @param index
	 *            the index of the item to be set
	 * @param value
	 ...
	 * @param dir
	 *            the text's direction. If {@code null} and direction estimation is turned off, direction is ignored.
	 */
