...
public class GinjectorGenerator extends Generator {
	...
	public String generate(TreeLogger logger, GeneratorContext context, String typeName) throws UnableToCompleteException {
		...
		try {
			...
			this.logger.log(TreeLogger.ERROR,
					String.format("Unable to load ginjector type [%s], " + "maybe you haven't compiled your client java sources?", typeName), e);
		...
		Module module = new GinjectorGeneratorModule(logger, context, ginjectorInterface, getModuleClasses(ginjectorInterface));
	 ...
	 * 
	 * @param logger
	 *            logger for errors that occur during class loading
	 * @param context
	 *            generator context in which classes are loaded
	...
	private ClassLoader createGinClassLoader(TreeLogger logger, GeneratorContext context) {
		...
		exceptions.add("javax.annotation"); // Need the non-super-source version during generation.
		...
		for (String moduleName : propertyModuleNames) {
			try {
				...
				logger.log(TreeLogger.ERROR,
						String.format("Unable to load gin module type [%s], " + "maybe you haven't compiled your client java sources?", moduleName), e);
				throw new UnableToCompleteException();
			}
		}
	}

	private Set<String> getPropertyModuleNames(Class<?> ginjectorType) throws UnableToCompleteException {
	...
	private Set<String> getValuesForProperty(String propertyName) {
		try {
			...
			return new LinkedHashSet<String>(propertyOracle.getConfigurationProperty(propertyName).getValues());
	...
	/**
	 * Loads the class with the given name. If the provided name is a source name this method tries to find the appropriate class.
	 * 
	 * <p>
	 * For example, given a string {@code A.B.C}, this method will try to load (in this order, returning upon found class): {@code A.B.C}, {@code A.B$C} and
	 * {@code A$B$C}.
	 * 
	 * @param requestedClass
	 *            binary or source name of class to be loaded
	 * @param initialize
	 *            whether to initialize the loaded class or not
	 * @return loaded class
	 * @throws ClassNotFoundException
	 *             if no class could be found for the provided name
	 */
