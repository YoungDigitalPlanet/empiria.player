...
public class SelectionModuleViewImplTest {
	...
	public void testSetItemDisplayedName(){
		...
		when(gridGenerator.createItemDisplayElement(Mockito.any(Element.class))).thenReturn(gridElement);
		
		
		selectionView.setItemDisplayedName(item, position);
		//then
		
		verify(gridGenerator).createItemDisplayElement(Mockito.any(Element.class));
	...
	public void testSetChoiceOptionDisplayedName(){
		...
		when(gridGenerator.createChoiceDisplayElement(Mockito.any(Element.class))).thenReturn(gridElement);
		
		
		selectionView.setChoiceOptionDisplayedName(item, position);
		//then
		
		verify(gridGenerator).createChoiceDisplayElement(Mockito.any(Element.class));
		verify(grid).setWidget(0, 0, gridElement.asWidget());
	}
