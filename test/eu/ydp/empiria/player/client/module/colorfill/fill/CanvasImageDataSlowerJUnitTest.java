...
public class CanvasImageDataSlowerJUnitTest {
	...
	public void setUp() throws Exception {
		...
		when(context.getImageData(0, 0, width, height))
	...
	public void shouldReturnColorFromImageDataOnPosition() throws Exception {
		...
		when(imageData.getRedAt(x, y))
			.thenReturn(red);
		
		when(imageData.getGreenAt(x, y))
			.thenReturn(green);
		
		when(imageData.getBlueAt(x, y))
		.thenReturn(blue);
		
		when(imageData.getAlphaAt(x, y))
		.thenReturn(alpha);
