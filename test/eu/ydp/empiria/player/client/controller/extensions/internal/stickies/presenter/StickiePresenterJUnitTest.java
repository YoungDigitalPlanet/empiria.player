...
public class StickiePresenterJUnitTest {
	...
	private @Mock
	IStickieProperties stickieProperties;
	private @Mock
	StickieMinimizeMaximizeController minimizeMaximizeController;
	private @Mock
	StickieRegistration stickieRegistration;
	private @Mock
	StickieViewPositionFinder positionFinder;
	private @Mock
	IStickieView stickieView;
	...
	public void setUp() throws Exception {
		...
		when(stickieView.getStickieDimensions()).thenReturn(stickieDimensions);

		when(stickieView.getParentDimensions()).thenReturn(parentDimensions);
	}
	...
	public void shouldMinimizeStickieWhenIsMaximized() throws Exception {
		...
		when(minimizeMaximizeController.positionMinimizedStickie(stickieDimensions)).thenReturn(newPosition);
	...
	public void shouldMaximizeStickieWhenIsMinimized() throws Exception {
		...
		Point<Integer> newPosition = new Point<Integer>(987, 654);
		when(minimizeMaximizeController.positionMaximizedStickie(stickieDimensions, parentDimensions)).thenReturn(newPosition);
	...
	public void shouldCenterPositionToViewAndCorrectIfNeeded() throws Exception {
		...
		when(positionFinder.calculateCenterPosition(stickieDimensions, parentDimensions)).thenReturn(centerPosition);

		Point<Integer> correctedPosition = new Point<Integer>(666, 777);
		when(positionFinder.refinePosition(centerPosition, stickieDimensions, parentDimensions)).thenReturn(correctedPosition);
	...
	public void shouldCorrectCurrentStickiePosition() throws Exception {
		...
		when(positionFinder.refinePosition(currentPosition, stickieDimensions, parentDimensions)).thenReturn(correctedPosition);
	...
	public void shouldResetMinimizedStickiePositionAndUpdateStickiePosition() throws Exception {
		...
		when(positionFinder.refinePosition(newPosition, stickieDimensions, parentDimensions)).thenReturn(correctedPosition);
