...
public class StickieViewPositionFinderTest {
	...
	public void setUp() {
		...
		stickieDimensions = new ContainerDimensions.Builder().width(10)
				.height(10)
				.absoluteLeft(0)
				.absoluteTop(0)
				.build();

		pageDimensions = new ContainerDimensions.Builder().width(100)
				.height(100)
				.absoluteLeft(10)
				.absoluteTop(10)
				.build();

		playerDimensions = new ContainerDimensions.Builder().width(1000)
				.height(1000)
				.absoluteLeft(0)
				.absoluteTop(0)
				.build();
		
		when(sizeHelper.getPlayerContainerDimensions())
			.thenReturn(playerDimensions);
	}
	...
	public void shouldCalculateCenterPosition() throws Exception {
		...
		when(positionFinder.getCenterPosition(stickieDimensions.getWidth(), pageDimensions.getAbsoluteLeft(), Axis.HORIZONTAL))
			.thenReturn(centerXCoordinate);

		Integer centerYCoordinate = 1231231;
		when(positionFinder.getCenterPosition(stickieDimensions.getHeight(), pageDimensions.getAbsoluteTop(), Axis.VERTICAL))
			.thenReturn(centerYCoordinate);
